...
const CalendarMonthCell = ({ date, dayEvents, isCurrentMonth }) => {
  ...
  return (
    <div
      className={`border border-(--border) p-2 min-h-32 ${
        isCurrentMonth ? "bg-(--background-card)" : "bg-(--background-tertiary)"
      } ${dayEvents.length > 0 ? "cursor-pointer" : ""}`}
      onClick={() => dayEvents.length > 0 && setShowModal(true)}
    >
      {/* Número do dia */}
      <p className="text-xs text-(--foreground-subtle)">
        {isCurrentMonth ? date.getDate() : ""}
      ...
      {/* Mobile (<648px): apenas destaque */}
      <div className="block md:hidden">
        {dayEvents.length > 0 && (
          <span className="mt-1 block w-full h-2 rounded bg-(--calendar)"></span>
        )}
      </div>

      {/* Desktop (≥648px): lista de eventos */}
      <div className="hidden md:flex flex-col gap-1 mt-1 max-h-24 overflow-y-auto">
        {dayEvents.slice(0, 3).map((ev, idx) => (
          <span
            key={idx}
            className={`text-xs rounded px-1 truncate ${
